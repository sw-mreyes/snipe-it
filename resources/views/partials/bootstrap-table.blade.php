...
<script nonce="{{ csrf_token() }}">
    ...
    function assetTagLinkFormatter(value, row) {
        if ((row.asset) && (row.asset.id)) {
            ...
            if (row.tag) {
                ...
                const pre = parts[0];
                const num = parts[1];
                if (pre === "SW") return '<a href="{{ url('/') }}/hardware/' + row.id + '"> ' + value + '</a>';
                let dest = pre === "AC" ? 'accessories' : pre === 'CM' ? 'components' : pre === 'CS' ? 'consumables' : 'ERROR';
                return '<a href="{{ url('/') }}/' + dest + '/' + num + '"> ' + value + '</a>';
            } else return value;
        }
    }
   ...
   function imageFormatter(value, row) {
    ...
    function acID2TagFormatter(data) {
        return id2tag('AC', data);
    }

    function cmID2TagFormatter(data) {
        return id2tag('CM', data);
    }

    function csID2TagFormatter(data) {
        return id2tag('CS', data);
    }
